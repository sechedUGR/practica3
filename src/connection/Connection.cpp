...
# include "../../include/model/RemotePlayer.h"
...
void ParchisRemote::sendHelloMaster(string ip, int port){
   ...
   if (status != Socket::Done){
      ...
      cout << "ip: " << ip << " port: " << port << " online_version: " << ONLINE_VERSION << " ninja_version: " <<
         NINJA_VERSION << endl;
   }
}
...
void ParchisRemote::sendNinjaStatus(int ninja_games, int random_games, int private_games){
   ...
   if (status != Socket::Done){
      ...
      cout << "ninja_games: " << ninja_games << " random_games: " << random_games << " private_games: " << private_games
         << endl;
   }
}
...
void ParchisRemote::packet2HelloMaster(Packet& packet, string& ip_addr, int& port, int& online_version,
                                       int& ninja_version){
   packet >> ip_addr;
   packet >> port;
   packet >> online_version;
   packet >> ninja_version;
   cout << "ip_addr: " << ip_addr << " port: " << port << " online_version: " << online_version << " ninja_version: " <<
      ninja_version << endl;
}
...
NinjaServer::NinjaServer(const int& myport, const string& my_contact_ip) : reviser(&NinjaServer::reviserLoop, this),
                                                                           ...
                                                                           master_thread(
                                                                              &NinjaServer::masterReceiver,
                                                                              this)
// & NinjaServer::masterReceiver, this)
{
...
void NinjaServer::acceptationStep(shared_ptr<ParchisServer> server){
   ...
   if (reserved_ips.find(con) == reserved_ips.end()){
      cout << COUT_RED_BOLD << "Connection from " << server->getRemoteAddress() << ":" << server->getRemotePort() <<
         " is not allowed." << COUT_NOCOLOR << endl;
...
void NinjaServer::queueRandomMatchGame(shared_ptr<ParchisServer> server, Packet& packet, shared_ptr<Thread> thread){
   ...
   if (!random_match_games.empty() && random_match_games.back().waiting_for_players){
      ...
      shared_ptr<RemotePlayer> p1 = make_shared<RemotePlayer>(random_match_games.back().name_p1,
                                                              random_match_games.back().connection_p1);
      shared_ptr<RemotePlayer> p2 = make_shared<RemotePlayer>(random_match_games.back().name_p2,
                                                              random_match_games.back().connection_p2);
      ...
      random_match_games.back().aux_thread = thread;
      // Para evitar que se intente destruir a sí misma cuando se libere el shared_ptr.
...
void NinjaServer::queuePrivateRoomGame(shared_ptr<ParchisServer> server, Packet& packet, shared_ptr<Thread> thread){
   ...
   if (curr_room != private_room_games.end()){
      ...
      if (room->waiting_for_players){
         ...
         server->sendErrorMessage(ERR_FULL_ROOM,
                                  "La sala indicada ya existe y está completa. Por favor, indica otro nombre de sala.");
      ...
      private_room_games[room_name].aux_thread = thread;
      // Para evitar que se intente destruir a sí misma cuando se libere el shared_ptr.
      private_room_games_mutex.unlock();
...
void NinjaServer::reviseNinjaThreadsStep(){
   ...
   cout << COUT_BLUE_BOLD + "Current connections: " << ninja_games.size() << " (" << nremoves << " removed)" +
      COUT_NOCOLOR << endl;
}
...
void NinjaServer::reviseRandomMatchThreadsStep(){
   ...
   random_match_games_mutex.unlock();
   cout << COUT_BLUE_BOLD + "Current connections: " << random_match_games.size() << " (" << nremoves << " removed)" +
      COUT_NOCOLOR << endl;
}
...
void NinjaServer::revisePrivateRoomThreadsStep(){
   ...
   private_room_games_mutex.unlock();
   cout << COUT_BLUE_BOLD + "Current connections: " << private_room_games.size() << " (" << nremoves << " removed)" +
      COUT_NOCOLOR << endl;
}
...
void NinjaServer::reviseDeadThreadsStep(){
   ...
   cout << COUT_BLUE_BOLD + "Current connections: " << dead_threads.size() << " (" << nremoves << " removed)" +
      COUT_NOCOLOR << endl;
}
...
void MasterServer::handleNinjaConnection(shared_ptr<ParchisServer> server, Packet& packet){
   ...
   if (ninja_version != NINJA_VERSION){
      server->sendErrorMessage(
         ERR_UPDATE, "Hay una versión más reciente del servidor ninja. Es necesario actualizar el repositorio.");
      return;
   }
   if (online_version != ONLINE_VERSION){
      server->sendErrorMessage(
         ERR_UPDATE,
         "Hay una actualización disponible del juego y del modo online. Es necesario actualizar el repositorio.");
      return;
   }
   ...
   if (allowed_ninja_ips.find(ip_addr) != allowed_ninja_ips.end()){
      ...
      cout << COUT_GREEN_BOLD << "New ninja added from " << server->getRemoteAddress() << ":" << server->getRemotePort()
         << COUT_NOCOLOR << endl;
...
void MasterServer::handleClientConnection(shared_ptr<ParchisServer> server, Packet& packet){
   ...
   if (version != ONLINE_VERSION){
      server->sendErrorMessage(
         ERR_UPDATE,
         "Hay una versión más reciente del juego. Es necesario actualizar el repositorio para jugar online, y también recomendable para el juego local. Recuerda: git pull upstream master (tras haber seguido los pasos del tutorial).");
      return;
   }
...
void MasterServer::reserveNinjaGame(shared_ptr<ParchisServer> server){
   ...
   for (auto it = ninja_connections.begin(); it != ninja_connections.end(); ++it){
      ...
      if ((*it).connection->isConnected()){
         ...
         if (message_kind == NINJA_STATUS){
            ...
            cout << COUT_RED_BOLD << "Error: El servidor ninja ha enviado un mensaje inesperado." << COUT_NOCOLOR <<
               endl;
         }
      }
      else{
   ...
   if (lowest_occupation_it != ninja_connections.end()){
      if (lowest_occupation < MAX_ALLOWED_NINJA_GAMES){
         ...
         instant_send_receive_mutex1.lock();
         (*lowest_occupation_it).connection->sendReserveIp(server->getRemoteAddress().toString(),
         ...
         if (message_kind == OK_RESERVED){
            ...
            server->sendErrorMessage(ERR_COULDNT_RESERVE,
                                     "No se ha podido asignar un servidor ninja. Inténtalo de nuevo, por favor. Si el problema persiste avisa a tus profesores.");
         }
      }
      else{
...
void MasterServer::reserveRandomGame(shared_ptr<ParchisServer> server){
   ...
   if (last_random_assigned_connection != nullptr){
      instant_send_receive_mutex1.lock();
      last_random_assigned_connection->connection->sendReserveIp(server->getRemoteAddress().toString(),
                                                                 server->getRemotePort());
      // Wait for the OK
      Packet packet;
      MessageKind message_kind;
      ...
      if (message_kind == OK_RESERVED){
         ...
         server->sendErrorMessage(ERR_COULDNT_RESERVE,
                                  "No se ha podido asignar un servidor. Inténtalo de nuevo, por favor. Si el problema persiste avisa a tus profesores.");
         last_random_assigned_connection = nullptr;
      }
   }
   else{
...
void MasterServer::reviserLoop(){
   while (is_running){
         ...
         if (!(*it_ninjas).connection->isConnected()){
            ...
            if (message_kind == NINJA_STATUS){
               ...
               if (ninja_games < MAX_ALLOWED_NINJA_GAMES && queued_connections.size() > 0){
                  // Asignarle el servidor ninja.
                  instant_send_receive_mutex1.lock();
                  (*it_ninjas).connection->sendReserveIp(queued_connections.front()->getRemoteAddress().toString(),
                                                         queued_connections.front()->getRemotePort());
                  // Wait for the OK
                  Packet packet;
                  MessageKind message_kind;
                  message_kind = (*it_ninjas).connection->receive(packet);
                  ...
                  if (message_kind == OK_RESERVED){
                     ...
                     cout << COUT_RED_BOLD << "Ha fallado la asignación de un servidor ninja al cliente encolado." <<
                        COUT_NOCOLOR << endl;
                  }
               }
            }
      ...
      cout << COUT_BLUE_BOLD + "Current connections: " << ninja_connections.size() << " (" << nremoves << " removed)" +
         COUT_NOCOLOR << endl;
      sleep(seconds(10));
   }
}
