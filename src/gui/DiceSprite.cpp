#include "../../include/gui/DiceSprite.h"
...
DiceSprite::DiceSprite(const Texture& t, int n, color c) : ClickableSprite(t){
   this->number = n;
   this->c = c;
   this->setTextureRect(IntRect(num2textrec.at(n).at(0), num2textrec.at(n).at(1), num2textrec.at(n).at(2),
                                num2textrec.at(n).at(3)));
...
void DiceSprite::setNumber(int n){
   this->number = n;
   if (selected){
      this->setTextureRect(IntRect(num2selectedrec.at(n).at(0), num2selectedrec.at(n).at(1),
                                   num2selectedrec.at(n).at(2), num2selectedrec.at(n).at(3)));
   }
   else{
      this->setTextureRect(IntRect(num2textrec.at(n).at(0), num2textrec.at(n).at(1), num2textrec.at(n).at(2),
                                   num2textrec.at(n).at(3)));
...
void DiceSprite::onHoverAction(Window& container){
   if (hovered && number != -1){
      ...
      if (locked || !enabled){
         ...
         this->setTextureRect(IntRect(num2selectedrec.at(number).at(0), num2selectedrec.at(number).at(1),
                                      num2selectedrec.at(number).at(2), num2selectedrec.at(number).at(3)));
         gui->setHandCursor();
      }
   }
   else{
      this->setTextureRect(IntRect(num2textrec.at(number).at(0), num2textrec.at(number).at(1),
                                   num2textrec.at(number).at(2), num2textrec.at(number).at(3)));
   }
}
