...
# include "../include/gui/IncludesSFML.h"
 ...
 * @param ports
 */
void readAllowedMasters(const string& file_name, vector<string>& ips, vector<int>& ports){
   ...
   for (i = 0; i < ips.size(); i++){
      ...
      try{
         ...
         do{
            ...
            {
               cout << COUT_RED_BOLD << ParchisClient::packet2errorMessage(packet) << COUT_NOCOLOR << endl;
               if (message_kind == ERR_UPDATE){
                  exit(1);
               }
               else throw runtime_error("Could not connect to master server: " + to_string(message_kind));
            }
            else if (message_kind ==
               QUEUED) // Si está encolado, sigue esperando y se especifica el lugar en la cola de espera.
            {
               cout << "Lo sentimos. El servidor está completo en este momento. Se te ha puesto en cola." << endl;
               int queue_pos = ParchisClient::packet2queuePos(packet);
               cout << "Tu posición en la cola es: #" << queue_pos << endl;
            }
         }
         while (message_kind != ACCEPTED);
         ParchisClient::packet2acceptedIp(packet, ip_addr, port);
         break;
      }
      // Si no se puede conectar al servidor
      catch (...){
         cout << COUT_RED_BOLD << "Could not connect to master server: " << ips[i] << ":" << ports[i] << COUT_NOCOLOR
            << endl;
      }
   }
   // Si no ha sido posible conectarse con ningún servidor...
   if (i == ips.size()){
      cout << COUT_RED_BOLD << "No se ha podido conectar a ningún servidor. Avisa a tus profesores cuanto antes."
         << COUT_NOCOLOR << endl;
   }
}
 ...
 */
vector<string> readAllowedNinjas(const string& file_name){
...
int main(int argc, char const* argv[]){
   ...
   for (int i = 1; i < argc; i++){
      ...
      // Si los argumentos no son correctos....
         ...
         cout << "Usage: " << argv[0]
            <<
            " --p1 <type=GUI|AI|Client|Server|Ninja> (id=0) (name=J1) --p2 <type=GUI|AI|Client|Server|Ninja> (id=0) (name=J2) --ip <ip>  [Optional] --port <port>  [Optional] --board <config=GROUPED> --no-gui  [Optional] --playground [Optional]"
            << endl;
         return -1;
      }
   }
   ...
   if (argc == 1){
      // Si no se pasan argumentos se crea la ventana para la selección de modo de
      // juego, que se encargará de asignar los parámetros del juego a las variables.
   ...
   bool is_remote = (type_j1 == "REMOTE" || type_j1 == "SERVER" || type_j1 == "NINJA") or
      (type_j2 == "REMOTE" || type_j2 == "SERVER" || type_j2 == "NINJA");
   ...
   if (master_server){
      ...
      // Leo las máquinas que están autorizadas para ser ninjas e inicializo el servidor.
      ...
      // Leo MY_PUBLIC_IP_FILE y cojo el primer string (ip_address)
      ...
      for (int i = 0; i < ips.size(); i = (i + 1) % ips.size()){
         ...
         try{
            ...
            cout << COUT_RED_BOLD << "No se pudo contactar con el master en " << ips[i] << ":" << ports[i]
               << COUT_NOCOLOR << endl;
         }
      }
   }
   // Si soy servidor
      ...
      if (playground){
         // Modo playground fuerza a que todos sean GUI.
         type_j1 = "GUI";
         type_j2 = "GUI";
         gui = true;
      }
      if (type_j1 == "GUI"){
         // Inicializo jugador 1 GUI
         p1 = make_shared<GUIPlayer>(name_j1, id_j1);
      }
      else if (type_j1 == "AI"){
         // Inicializo jugador 1 AI
         p1 = make_shared<AIPlayer>(name_j1, id_j1);
      }
      else if (type_j1 == "LNINJA"){
         // Inicializo jugador 1 LNINJA
         p1 = make_shared<Ninja>(name_j1, id_j1);
      }
      else if (type_j1 == "REMOTE"){
         // Inicializo j1 a cliente remoto
         shared_ptr<ParchisClient> client = make_shared<ParchisClient>();
      ...
      if (type_j2 == "GUI"){
         // Inicializo el jugador 2 GUI
         p2 = make_shared<GUIPlayer>(name_j2, id_j2);
      }
      else if (type_j2 == "AI"){
         // Inicializo el jugador 2 AI
         p2 = make_shared<AIPlayer>(name_j2, id_j2);
      }
      else if (type_j2 == "LNINJA"){
         // Inicializo el jugador 2 LNINJA
         p2 = make_shared<Ninja>(name_j2, id_j2);
      }
      else if (type_j2 == "REMOTE"){
         // Inicializo j2 a cliente remoto
         shared_ptr<ParchisClient> client = make_shared<ParchisClient>();
         // Comienzo la conexión al puerto e ip especificadas.
         client->startClientConnection(ip, port);
         // Mando los parámetros del juego
   ...
   // Una vez creados los jugadores, se crea el objeto parchís con la
   // configuración incial establecida.
   // config = ALL_AT_HOME;
   // config = ALL_AT_GOAL;
   //config = CORRIDORS_ONE_PIECE;
   //config = CORRIDORS_TWO_PIECES;
   //config = DRAW_ONE_PIECE;
   //config = DRAW_TWO_PIECES;
   config = GROUPED2;
