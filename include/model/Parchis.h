...
# include "GUIPlayer.h"
# include "Player.h"
# include "../../src/personal/AIPlayer.h"
# include "PowerBar.h"
...
class ParchisBros{
   /**
    * Clase que gestiona la iteración sobre los hijos (bros) de un nodo de Parchis (parent).
    * A la hora de recorrer los hijos de un tablero, tenemos dos opciones:
    * - Iterar hijo a hijo con el iterador de ParchisBros (nos permite ir uno a uno y parar de generar nodos antes si lo consideramos oportuno).
    * - Obtener todos los hijos de un tablero como un vector de ParchisSis (permite tener todos los nodos cargados a la vez y hacer distintas operaciones con ellos, como comparaciones antes de seguir explorando).
    */
private:
...
public:
   ...
   public:
       ...
       * @param last_dice
       */
      Iterator(ParchisBros& container, shared_ptr<Parchis>& p, color last_c_piece = none, int last_id_piece = -1,
               int last_dice = -1);
...
class ParchisSis{
   private:
   /**
    * Estructura para almacenar nodos de Parchis, junto con el movimiento que lleva a ese nodo.
    * Colaboran con los ParchisBros para obtener un vector con todos los nodos hijos de un tablero concreto.
    * A la hora de recorrer los hijos de un tablero, tenemos dos opciones:
    * - Iterar hijo a hijo con el iterador de ParchisBros (nos permite ir uno a uno y parar de generar nodos antes si lo consideramos oportuno).
    * - Obtener todos los hijos de un tablero como un vector de ParchisSis (permite tener todos los nodos cargados a la vez y hacer distintas operaciones con ellos, como comparaciones antes de seguir explorando).
    */
...
public:
   ...
   */
   ...
   inline color getCurrentMainColor() const{
      return this->getPlayerColors(current_player)[0];
   }
   ...
   /**
    * @brief Obtener la lista completa de hijos del nodo actual. Esta función aumenta la cuenta de nodos generados con la rama entera.
    * IMPORTANTE: No es recomendable usar a la vez getChildren (iterador) y getChildrenList (lista completa). Se contarían los nodos generados dos veces.
    *
    * @return ParchisBros
    */
   vector<ParchisSis> getChildrenList() const;
